...
GreekLatinSquare generateOrthogonalByExactCover(GreekLatinSquare * source)
{
    ...
    int line_index = 0;
    VectorInt force_included_values = defaultVectorIntCalloc(0, 0);
    ...
    VectorInt exact_cover = makeExactCover(&matrix, &force_included_values);
    destructVectorInt(&force_included_values);
  //  printfVectorInt("", &exact_cover);
    sortVectorInt(&exact_cover, &intLessComparator);
    ...
    for (int i = 0; i < exact_cover.getSize(&exact_cover); ++i)
    {
        if (i % n == 0)
            continue;
        int k = -1;
        for (k = 0; k < n2; ++k)
            if (*y(&matrix, *exact_cover.at(&exact_cover, i), k / n, k % n) == 1)
            {
                break;
            }

        assert(k != n2);
        *atGreekLatinSquare(&res, i / n, i % n) = k % n + 1;
    }
